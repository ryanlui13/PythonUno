...
def animate_card(screen, card_back, start_pos, end_pos, TABLE_GREEN, shared_deck, deck_pos, player_hand, computer_hand, pile, player_y, computer_y, card_Spacing, steps=20, delay_ms=15):
        sx, sy = start_pos 
        ex, ey = end_pos 
        ...
        for i in range(1, steps+1):
            ...
            screen.blit(shared_deck, deck_pos) 

            draw_hand(screen, player_hand, player_y, card_spacing, show_face=True)
            draw_hand(screen, computer_hand, computer_y, card_spacing, show_Face=False)
...
def draw_hand(screen, hand_y, card_spacing, show_face=True):
    for i, card in enumerate(hand):
        s = 100 + i * card_spacing 
        if show_face:   #player's hand
            screen.blit(card.image, (x, y))
        else: 
            screen.blit(card_back, (x,y))

def distribute_cards_animated(shared_deck, hand, screen, start_pos, end_pos_y, table_color, deck_surface, deck_pos, owner="player"): #pass 7 cards to players
    """
        shared_deck: list of cards to draw from
        hand: player's hand 
        screen: pygame
        start_pos: (x,y) position of deck
        end_pos_y: y position for player's hand
        deck_surface: shared_deck image 
        deck_pos: shared_deck position 
        owner: who's the player 
    """
    for i in range(7):
        if not deck:    #see if any more cards left in the deck
            break 
        ...
        if owner == "player":
            animate_card(screen, card, start_pos, end_pos, table_color, shared_deck, deck_pos, mode="draw")
        else:
            animate_card(screen, card, start_pos, end_pos, table_color, shared_deck, deck_pos, mode="back") 
...
def main():
    ...
    #images
    sharedDeck_img = pygame.image.load("images/shared_deck.jpg")
